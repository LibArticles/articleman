...
export default class DataSetManager {
	static async getColumns() {
		const columns: Record<
			string,
			Record<string, GoogleAppsScript.Spreadsheet.Range>
		> = {};
	...
	async getDataSet(name: string) {
		return RawDataSetManager.getDataSet(name).then((set) =>
			set.getRange().getValues(),
		);
	}

	async updateDataSet(name: string, array: any[][]) {
		return RawDataSetManager.getDataSet(name).then((set) =>
			set.getRange().setValues(array),
		);
	}
}
