// TYPESCRIPT TYPE DEFINITIONS FOR SIALIZER WERE MADE BY Rafael Sliveira Cordeiro, @rafaelsc on GitHub, AND LIGHTLY MODIFIED BY BLUE LINDEN FOR THE VALENTECH FORK OF SIALIZER

declare module "@valentech/sializer" {

type BinaryData = Buffer;

type InternallySupportTypes =
  | null
  | undefined
  | string
  | number
 ...
 * ```
 */
interface ConstructorFactory<T, R extends InternallySupportTypes[] = InternallySupportTypes[]> {
  /**
 ...
 * ```
 */
const desia: <T>(data: BinaryData) => T;

/**
 * Array of default constructors used both by Sia and DeSia.
 */
const constructors: ReadonlyArray<ConstructorFactory<any, any>>;
