...
export default class ObjectManager {
  ...
  private findById(id: string) {
    ...
    for (const sheet of Object.values(this.data)) {
      for (const column of Object.values(sheet)) {
        ...
        const matches = column.range.createTextFinder(id).matchCase(true).matchEntireCell(true).findAll();
  ...
  async getObjectForId(id: string) {
    ...
    for (const column of Object.values(this.data[idRange.getSheet().getSheetId()])) {
